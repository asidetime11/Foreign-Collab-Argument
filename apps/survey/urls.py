from django.urls import path

from . import views


app_name = "survey"

urlpatterns = [
    path("", views.start, name="start"),
    path("consent/", views.consent, name="consent"),
    path("topic-order/", views.topic_order, name="topic_order"),
    path("round-transition/", views.round_transition, name="round_transition"),
    path("post/", views.post, name="post"),
    path("scale/<str:step>/", views.scale, name="scale"),
    path("text/<str:step>/", views.text_response, name="text_response"),
    path("mode/", views.mode_select, name="mode_select"),
    path("chat/", views.chat, name="chat"),
    path("english-paper/intro/", views.english_paper_intro, name="english_paper_intro"),
    path("english-paper/", views.english_paper, name="english_paper"),
    path("english-paper/draft/", views.english_paper_draft, name="english_paper_draft"),
    path("done/", views.done, name="done"),
    path("reset-self/", views.reset_self, name="reset_self"),
    path("quality-event/", views.quality_event, name="quality_event"),
]
